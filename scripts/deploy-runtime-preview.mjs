...
if (!isWorkersBuild || branch !== "preview/settlement-history-runtime") {
  process.exit(0);
}
...
const deploy = spawnSync(
  npx,
  ["wrangler", "deploy", "--config", "wrangler.preview-runtime.jsonc"],
  {
    encoding: "utf8",
    env: process.env,
  },
...
const previewUrl =
  "https://lightning-split-settlement-preview.thumbking-btc.workers.dev/";
let lastError;
for (let attempt = 1; attempt <= 12; attempt += 1) {
  try {
    ...
    if (response.ok && /LIGHTNING SPLIT|Lightning Split/u.test(body)) {
      console.log(`Verified runtime Preview URL: ${previewUrl}`);
      process.exit(0);
    }
    lastError = new Error(
